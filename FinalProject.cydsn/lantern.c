...
const uint8_t sineBrightness[256] = {
...
void lightLantern(uint8_t mode) {
    uint8_t disp;
    
    if (mode == 0) {                                // blue lantern
        for (disp = 0; disp < NUM_SCREENS; disp ++ ) {
            DMA_Wait_For_Completion(disp);
            clearScreen(disp, 0x3A2A);
        }
        for (disp = 0; disp < NUM_SCREENS; disp ++ ) {
            DMA_Wait_For_Completion(disp);
        ...
        for (disp = 0; disp < NUM_SCREENS; disp ++ ) {
            DMA_Wait_For_Completion(disp);
            drawRect(disp, 40, 279, 120, 419, 0x2E59);
        }
        for (disp = 0; disp < NUM_SCREENS; disp ++ ) {
            DMA_Wait_For_Completion(disp);
            drawRect(disp, 40, 99, 120, 179, 0x04B3);
        }
        for (disp = 0; disp < NUM_SCREENS; disp ++ ) {
            DMA_Wait_For_Completion(disp);
            drawRect(disp, 220, 279, 120, 179, 0x04B3);
        }
        for (disp = 0; disp < NUM_SCREENS; disp ++ ) {
            DMA_Wait_For_Completion(disp);
            drawRect(disp, 100, 219, 180, 239, 0x779E);
        }
        for (disp = 0; disp < NUM_SCREENS; disp ++ ) {
            DMA_Wait_For_Completion(disp);
            drawRect(disp, 100, 219, 360, 419, 0x779E);
        }
        for (disp = 0; disp < NUM_SCREENS; disp ++ ) {
            DMA_Wait_For_Completion(disp);
            drawRect(disp, 40, 99, 240, 359, 0x779E);
        }
        for (disp = 0; disp < NUM_SCREENS; disp ++ ) {
            DMA_Wait_For_Completion(disp);
            drawRect(disp, 220, 279, 240, 359, 0x779E);
        }
        for (disp = 0; disp < NUM_SCREENS; disp ++ ) {
            DMA_Wait_For_Completion(disp);
            drawRect(disp, 100, 219, 240, 359, 0xFFFF);
        }
        for (disp = 0; disp < NUM_SCREENS; disp ++ ) {
            DMA_Wait_For_Completion(disp);
            drawRect(disp, 100, 159, 240, 299, 0xCFFF);
        }
        for (disp = 0; disp < NUM_SCREENS; disp ++ ) {
            DMA_Wait_For_Completion(disp);
            drawRect(disp, 160, 219, 300, 359, 0xCFFF);
        }
